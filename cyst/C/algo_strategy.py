...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        if game_state.turn_number == 0:
            ...
            right_side_coord = [27,14]
            left_side_coord = [0,14]
            right_side_attackers = len(gamelib.advanced_game_state.AdvancedGameState.get_attackers(game_state, right_side_coord, 0))
            gamelib.debug_write('right_side_attackers = {}', right_side_attackers)
            left_side_attackers = len(gamelib.advanced_game_state.AdvancedGameState.get_attackers(game_state, left_side_coord, 0))
            gamelib.debug_write('left_side_attackers = {}', left_side_attackers)
            if left_side_attackers < right_side_attackers:
                if game_state.turn_number == 1:
                    self.attackRight = False
                else:
                    gamelib.debug_write('We should attack the left side!')
